...
public class G1_2042 {
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 0; i < M+K ; i++){
            ...
            if(act == 1){
                update_value(Integer.parseInt(st.nextToken()),Long.parseLong(st.nextToken()), seg_tree, N);
            }
            else if(act == 2){
    ...
    static void update_value(int idx, long value,long[] seg_tree ,int N){
        // 일단 해당 값을 갱신해줘야함
    ...
    static long query(int left ,int right , long[] seg_tree, int N){
        ...
        for(l+=N , r+=N ; l != r ; l>>=1 , r>>=1){
            // 왼쪽은 폐구간 이므로 오른쪽 자식을 가리키면 그대로 위로 가면 안된다. 오른자식 더하고 right구간으로 넘겨야 한다.
            if(l % 2 == 1) result+= seg_tree[l++];
            // 오른쪽은 개구간으로 잡았으므로 오른쪽 자식을 가리키면 왼자식만 더하고 구간을 왼쪽으로 좁혀야한다.
            if(r % 2 == 1) result += seg_tree[--r];
        }
        return result;
    }
}
